...
// Symbols declared global with .global/.globl. These directives do not
// generate machine instructions; they identify externally visible symbols.
set<string> global_labels;
...
int string_type(string s, int* previous_string_type)
{
    ...
    // Conventional MIPS assembly uses .global (or .globl) to declare
    // externally visible symbols, commonly: .global _start
    // The directive itself does not generate a machine instruction.
    if(s1 == ".global" || s1 == ".globl")
    {
        if(i < s.size()-1)
            return 5;
        return 0;
...
void type9(string opname, string operand, int operand_count, int line_count, int* valid, ofstream &file, ofstream &log, int write)
{
	if(operand_count == 3)
	{
        ...
        // MIPS branches use a PC-relative signed 16-bit offset:
        //
        //     offset = (target_address - (PC + 4)) / 4
        //
        // LABEL[lab] is a byte address and pc is the current
        // instruction index, so the current instruction address
        // is pc * 4.
        int target_address = LABEL[lab];
        int current_pc_address = pc * 4;

        int offset =
            (target_address - (current_pc_address + 4)) / 4;

        cout << "Branch instruction: " << opname << endl;
        cout << "Current PC: " << current_pc_address << endl;
        cout << "Target address: " << target_address << endl;
        cout << "Branch offset: " << offset << endl;

        // MIPS branch immediate is a signed 16-bit value.
        if(offset < -32768 || offset > 32767)
        {
            *valid = 0;
            cout << "Out of Range error : branch target is too far at line "
                 << line_count << endl;
            if(write == 1)
                log << "Out of Range error : branch target is too far at line "
                    << line_count << endl;
            return;
        }

        // Encode the signed offset as a 16-bit two's-complement value.
        uint16_t immediate =
...
void process(string s, int type, int line_count, int pc_counter, int* valid, ofstream &file, int pass, ofstream &log, int write)
{
    ...
    else if(type == 5)//.global/.globl directive
    {
        // The directive is metadata only and must not advance the PC.
        // Accept forms such as:
        //     .global _start
        //     .globl _start
        // Multiple symbols separated by commas are also accepted.
        string directive, symbols;
        size_t space_pos = s.find(' ');
        if(space_pos != string::npos)
            symbols = s.substr(space_pos + 1);

        // Remove trailing spaces.
        while(!symbols.empty() && isspace(static_cast<unsigned char>(symbols.back())))
            symbols.pop_back();
        ...
        {
            // Trim whitespace around each symbol.
            ...
            // MIPS labels in this assembler may start with a letter or '_'.
            if(!((isalpha(static_cast<unsigned char>(symbol[0])) || symbol[0] == '_')))
            {
                *valid = 0;
                cout << "syntax error at line " << line_count
                     << " : invalid global symbol " << symbol << endl;
                if(write == 1)
                    log << "syntax error at line " << line_count
                        << " : invalid global symbol " << symbol << endl;
                return;
...
void processFile(std::ifstream& inputFile, std::ofstream& outputFile) {
    ...
    for (auto& [k, v] : LABEL) {
        cout << k << " | " << v;
        if(global_labels.count(k) > 0)
            cout << " | GLOBAL";
        cout << endl;
    }

    cout << "\nglobal symbols declared with .global/.globl:" << endl;
    for (const auto& symbol : global_labels)
        cout << symbol << endl;
}
